...
build-container-operator-OCI: ## Builds components required for a cilium-operator OCI variant container. add by dw
	$(MAKE) $(SUBMAKEOPTS) -C $(SUBDIR_OPERATOR_CONTAINER) cilium-operator-OCI
...
install-container-binary-operator-OCI: ## Install binaries for all components required for cilium-operator OCI variant container. add by dw
	$(QUIET)$(INSTALL) -m 0755 -d $(DESTDIR)$(BINDIR)
	$(MAKE) $(SUBMAKEOPTS) -C $(SUBDIR_OPERATOR_CONTAINER) install-OCI
...
help: ## Display help for the Makefile, from https://www.thapaliya.com/en/writings/well-documented-makefiles/.
	...
	$(call print_help_line,"docker-operator-*-image","Build platform specific cilium-operator images(oci, alibabacloud, aws, azure, generic)")
