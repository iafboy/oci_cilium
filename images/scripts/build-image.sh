...
check_image_tag() {
  if [ -n "${MAKER_CONTAINER+x}" ] ; then
    ...
    echo "===========QUAY_PUBLIC_ACCESS_ONLY=true====="
...
#  docker buildx build --build-arg ftp_proxy=http://www-proxy-hqdc.us.oracle.com  --build-arg http_proxy=http://www-proxy-hqdc.us.oracle.com --build-arg https_proxy=http://www-proxy-hqdc.us.oracle.com:80 --output="${output}" "${tag_args[@]}" "${build_args[@]}"

#  docker buildx build --env http_proxy=http://www-proxy-hqdc.us.oracle.com --env https_proxy=http://www-proxy-hqdc.us.oracle.com:80 --output="${output}" "${tag_args[@]}" "${build_args[@]}"
...
#  docker buildx build  --output="type=image" "${tag_args[@]}" "${build_args[@]}"
