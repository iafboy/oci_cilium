...
func deriveVpcCIDRs(node *ciliumv2.CiliumNode) (primaryCIDR *cidr.CIDR, secondaryCIDRs []*cidr.CIDR) {
	...
	if len(node.Status.OCI.VNICs) > 0 {
		for _, vnic := range node.Status.OCI.VNICs {
			...
			if len(secondaryCIDRs) >= 1 {
				primaryCIDR = secondaryCIDRs[0]
				secondaryCIDRs = secondaryCIDRs[1:]
			}
		}
	}
	return
...
func (n *nodeStore) hasMinimumIPsInPool() (minimumReached bool, required, numAvailable int) {
	...
	if n.ownNode.Spec.IPAM.Pool != nil {
		...
		if n.conf.IPAMMode() == ipamOption.IPAMENI || n.conf.IPAMMode() == ipamOption.IPAMAzure || n.conf.IPAMMode() == ipamOption.IPAMOCI || n.conf.IPAMMode() == ipamOption.IPAMAlibabaCloud {
